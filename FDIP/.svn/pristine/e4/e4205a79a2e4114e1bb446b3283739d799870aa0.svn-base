...
public class Config {
	...
	public static String getConfigByKey(String key, String defaultValue, String... fillValues){
		String tValue = null;
		try {
			tValue = iResourceBundle.getString(key).trim();
		} catch (Exception e) {
		...
		tValue = StringUtils.isBlank(tValue)?defaultValue:tValue;
		tValue = MessageFormat.format(tValue, fillValues);
		return tValue;
	}
	
	public static String getDynamicConfigByKey(String key, String... fillValues){
		return getConfigByKey(key, "", fillValues);
	}
}
