...
public class FundService {
	...
	public Fund queryByCode(String fundCode){
		String key=""+(Constants.REDISBASEKEY+"Fund"+fundCode).hashCode();
		Fund fund=(Fund) RedisConfig.getObject(key);
		if(fund==null&&(fund=fundMapper.findByCode(fundCode))!=null){
			RedisConfig.set(key, fund);
		}
		return fund;
	}
