...
public class VerifyService {
	...
	public void singleIncome(String line){
		String currentDay=Constants._SDF_DATE.format(new Date());
		String[] infos = line.split("\\|");
		...
		if(currentDay.equals(accountFundAmount.getIncomeTime())){
			LOGGER.warn("客户{}今日分红数据已同步,无需处理",account3RD.getAccount().getName());
			return ;
		}
		...
		accountFundAmount.setIncomeTime(currentDay);
