...
public class FundTradeService {
	...
	public final BaseVo validate(BuyOrRedeForm form) {
		...
		if (null == account) {
			result.createError("account.notOpenAccount");
			return result;
		}else{
			if (account.getStatus().equals(Status.DISABLE)){
				result.createError("account.disabled");
				return result;
			}
		}
		Account3RD account3rd = getAccount3RD(form.getChannel(), form.getAccountNo());
		if (null == account3rd){
			result.createError("account.notOpenAccount");
			return result;
		}
	...
	public final BaseVo validate(String channelCode,String fundCode){
		BaseVo result = new BaseVo();
		Channel channel = getChannel(channelCode);
		if (null == channel) {
			result.createError("channel.illegal");
			return result;
		}
...
//	}
	...
	public final Date editableDeadline() {
		...
		if (now.get(Calendar.HOUR_OF_DAY) < Constants.DEFAULT_TIME_POINT && holidayService.isWorkday(now.getTime())) {
		} else {
		...
		LOGGER.info("获取可修改截至时间为: [{}]", Constants.SDF_DATETIME.format(now.getTime()));
		return now.getTime();
	}
	...
	public BaseVo fundStatus(String channel,String fundCode){
		LOGGER.info("基金状态, params:{},{}",channel,fundCode);
		return new BaseVo();
	}
...
//		Thread.sleep(50000);
		System.err.println(Constants.SDF_DATETIME.format(now.getTime())); 
	}
}
