...
public interface AccountFundAmountMapper {
	...
	@Insert("insert into account_fund_amount(channel,fund,account,amount,interestAmount,frozenAmount,income,createTime) values(#{channel},#{fund},#{account},#{amount},#{interestAmount},#{frozenAmount},#{income},#{createTime})")
	@Options(keyProperty="id",keyColumn="id",useGeneratedKeys=true)
	void insert(AccountFundAmount accountFundAmount);
	
	@Select("select * from account_fund_amount where account=#{account} and channel=#{channel} for update")
	...
	@Update("update account_fund_amount set amount=#{amount}, interestAmount=#{interestAmount},income=#{income},frozenAmount=#{frozenAmount},incomeTime=#{incomeTime} where id=#{id}")
